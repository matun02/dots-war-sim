...
- **受け入れ基準**:
  - [x] ドラッグで複数選択、右クリックで全員が直線移動
  - [x] 選択中ユニットの周りに枠が表示される
- **見積**: 1〜1.5 日
- **実績**: 30 分
- **学び**: 選択状態は Zustand store ではなく InputCollector 内に保持し、selectedIds を render に直接渡すパターンで十分だった。PixiJS v8 の Graphics で選択リングを描画する際は毎フレーム `clear()` + 再描画が必要。apply-inputs は structuredClone 済みの state を in-place 変更するため new object 生成不要。
...
- **受け入れ基準**:
  - [x] 山を迂回する
  - [x] 同 start/goal で 100 回呼んで結果が完全一致（決定論）
  - [x] 到達不能で null を返す
  - [x] 性能: 64x36 上で 1 経路 < 2ms
- **見積**: 2 日
- **実績**: 30 分
- **学び**: A* の整数コスト演算（10/14）でIEEE 754 由来の非決定論を完全に回避。TypedArray（Int32Array, Uint8Array）でgScore/closed管理すると64×36で十分高速。対角移動時は隣接2セルの通行可否チェックが必要（壁の角を斜めにすり抜ける問題の回避）。path追従のwaypoint閾値0.3はセル中心到達判定に適切。
...
- **受け入れ基準**:
  - [x] 2 ユニットが接触すると 1秒ほどで両方死ぬ（HP=1, attack=1, 0.5s 間隔）
  - [x] 中立都市にユニットが 30 tick（1 秒）滞在で占領される
  - [x] 敵都市は 60 + 30 tick で奪える
  - [x] 単体テスト: 占領進捗のリセット（敵が混在したら止まる）
- **見積**: 2 日
- **実績**: 30 分
- **学び**: TypeScript strict mode の `noUncheckedIndexedAccess` 有効時は配列の index アクセスが `T | undefined` になるため、`for...of` または `!` 非 null アサーションが必要。空間ハッシュは Map/Set 不可（CLAUDE.md §4.3）のため `EntityId[][]` で実装。戦闘テストではクールダウン中のユニットは攻撃しないが被攻撃は受ける点に注意（テスト期待値を正確に）。
