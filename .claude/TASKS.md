...
- **M4**: Phase2 完了（P2-T1〜T7） — ✅ 完了 (2026-06-03)（P2-T6 チュートリアルは未実装としてクローズ。それ以外は全完了）
...
| P2-T4.2 | 隊形配置・衝突分離・HP/サイズ3倍 | 2026-05-24 | `803b10c` | 9 件 (累計 186) |
| P2-T4.3 | 中央配置・後方都市・中立削除・HP5倍 | 2026-05-25 | `7c496ff` | 0 件 (累計 186) |
| P2-T5 | マップ追加（計5枚）+ マップ選択UI + ビューポートfit | 2026-05-30 | `ae87cd0` | 27 件 (累計 223) |
| P2-T7 | 勝利条件 80% 都市支配（影響マップ都市重みは P2-T4.1 で実装済・確認のみ） | 2026-06-03 | `60c7b17` | 4 件 (core 166) |
| P2-T6 | チュートリアル — ❌ 未実装としてクローズ（2026-06-03 判断・実装せず） | — | — | — |
...
Phase 2 全タスク（P2-T1〜T7 + T4.1〜T4.3）完了で M4 達成:
...
- ~~初回起動時にチュートリアルオーバーレイが表示される~~（P2-T6 未実装クローズ・対象外）
...
| P2-T4.3 | 中央配置・後方都市・中立削除・HP5倍 | P2-T4.2 | `packages/core` + `packages/maps` + `apps/web` ※3dir例外 | 3h |
...
推奨実行順: T1 → T2 → T3 → T4 → T4.1 → T4.2 → **T4.3** → T5 → T6 → T7

---

### 次タスク: Phase 3（P3-T1〜）

Phase 2（M4）はクローズ。P2-T6 チュートリアルは未実装としてクローズしたため、Phase 2 の残タスクはなし。
次は Phase 3（マルチプレイ / M5）。詳細は `archive/future-phases.md` 参照。

> P2-T6（チュートリアル）を未実装でクローズしたことに伴い、当初予定していた
> 「`Tutorial.svelte` の勝利条件文言更新」も不要（ファイル自体を作らない）。
> チュートリアルを将来再開する場合は文言を「80% の都市を支配 or 全ユニットを殲滅で勝利」とすること。

---

プロンプトファイル: `archive/task-prompts/P2-T1-prompt.md` 〜 `P2-T7-prompt.md`（+ `P2-T4.3-prompt.md`）
