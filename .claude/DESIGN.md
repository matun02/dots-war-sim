...
```
[ゲーム開始時：各プレイヤーの自陣半分に light ユニット 10 体を初期配置]
    ↓
[都市が時間経過でユニットを生産（死亡ユニットの補充）]
    ↓
[プレイヤーがユニットを選択]
    ↓
...
- 各陣営ごとに 1 枚
- ユニット・都市位置から **円形フォールオフ**（半径 10 セル、線形減衰）で影響を加算
  - light=100, heavy=200, city_weight=300
  - 距離 d のセル: `weight * (R² - d²) / R²`（R=10）
- 3 回ガウシアン平滑化（3x3 box blur）で等高線を滑らかに
- AI 判断・前線描画に利用
...
- マーチングスクエア法で線を生成 → PixiJS の Graphics で描画（白半透明 2px, alpha 0.3）
- 前線は **両軍の境界線** として表示される（ユニットを囲むリングではない）
- ユニット・都市の円形影響により、後方領域は自然に自軍領土として認識される
...
| 2026-05-24 | 1.2.0 | P2-T4.1: §1.2 初期ユニット配置を追記。§8.1 影響マップを円形フォールオフ(R=10)+3回平滑化に刷新。§8.3 前線描画の動作仕様を明確化 |
