...
## P2-T5: マップ追加（計5枚）+ マップ選択UI + ビューポートfit ✅ 完了 (2026-05-30, commit `ae87cd0`)

- **目的**: first-blood に加え4マップを追加し、タイトル画面で選択可能にする。あわせて任意サイズのマップが画面にフィットするよう描画をビューポート対応化（ユーザー追加要望「比率や構成を変えずにフルスクリーン対応」）。
- **作業内容**:
  1. `packages/maps/src/data/` に4マップJSON追加。一時生成スクリプトで terrain 配列を生成し、heavy(=平地のみ通行)のフラッドフィルで全都市到達性・左右ミラー/90°回転対称・都市が平地上、を自動検証してから出力（スクリプトはコミット前に削除）
     - `bridge` (48×36, 6都市): 中央水域帯を平地橋1本で接続。チョーク戦
     - `four-corners` (48×48, 8都市): 90°回転対称、各プレイヤーが対角2都市+中央十字。森ピンウィール
     - `strait` (64×36, 6都市): 中央水縦壁+上下に幅3セルの海峡。挟撃
     - `corridor` (64×36, 6都市): 山脈で3横回廊に分離、両端に平地縦連結部。light=山越え可/heavy=回廊内のみ
  2. `index.ts` — 4マップ export + `MAP_CATALOG`(選択UIメタ) + `getMapJson(id)`(リゾルバ、未知idはfirst-bloodにフォールバック)
  3. `Title.svelte` — `MAP_CATALOG` からマップ選択リスト(ラジオ・選択ハイライト)、`onstart(mapId)`
  4. `App.svelte` — `startGame(mapId)` 化、`getMapJson` で解決、rematch は同マップ、replay も録画時マップで再生
  5. `player.ts` — `getMapJson(replay.mapId)` で再シミュレーション（従来 first-blood 固定だったリプレイバグを修正。シグネチャ不変で既存テスト4件は不変）
  6. `stage.ts` — `fitStageToWorld()` 追加。`app.stage` に均一スケール+中央オフセット(contain/レターボックス)を適用、リサイズで再フィット。`App.svelte` の入力(`toWorld`)は同じ transform で逆射影
  7. `loader.test.ts` — 各マップのロード/範囲/境界/対称性/heavy連結性 + MAP_CATALOG/getMapJson のテスト27件追加
- **成果物**: 新規4 JSON、編集6ファイル（index.ts, loader.test.ts, Title.svelte, App.svelte, stage.ts, player.ts）。2ディレクトリ(`packages/maps`+`apps/web`)
- **受け入れ基準**:
  - [x] 4新規マップが `loadMap()` でロード可能（バリデーション通過）
  - [x] `index.ts` から全マップ export
  - [x] タイトルでマップ選択→選択マップでゲーム開始
  - [x] 各マップが対称（公平）で異なる戦略地形、heavy通行可能な平地が十分
  - [x] 任意サイズのマップが比率を保ってビューポートにフィット（ウェブテストで全5マップ確認: 均一スケール・中央寄せ・収納・座標変換）
  - [x] typecheck / lint / test 全 green（合計 223 件）
- **実績**: 約2h
- **学び**:
  - heavy は地形コスト表(`astar.ts` `TERRAIN_COST_HEAVY=[1,-1,-1,-1,1]`)で平地(0)のみ通行可。森・山も不可。マップは陣地↔都市↔係争地を連続平地で繋ぐ必要があり、生成スクリプトにフラッドフィル連結性アサートを内蔵すると安全
  - 「中立都市」は P2-T4.3 で削除済み（`init.ts` owner決定が `spawn ?? owner ?? 0`、未指定は P0 所有）。プロンプトの中立都市は両者均等割当に読み替えて対称性を確保
  - フルスクリーン対応は全描画要素が `app.stage` 配下なら、stage 全体のスケール+オフセットで比率・構成を変えず一括フィット可能。入力はレンダリングと同じ transform で逆射影すれば座標が常に一致（ウェブテストで選択ボックス寸法=ドラッグ範囲を確認）
  - リプレイ map は `replay.mapId` から解決すべき（従来 first-blood 固定）。`getMapJson` を maps に集約し player のシグネチャを変えないことで、新機能追加と既存テスト不変を両立
